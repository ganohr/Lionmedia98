...
$example_update_checker = new ThemeUpdateChecker(
	...
	'lionmedia98',
	 ...
	 //JSONファイルのURL
	'https://lionmedia98.ganohr.net/update/lionmedia98-info.json'
);
...
function fit_post_cutomizer( $wp_customize ) {
	...
	*/
	// ORIGINAL:END
	// Lionmedia98:000:START Customize
...
function fit_document_title( $title ) {
	...
	// Lionmedia98:v002:START seo fix
	$seo_title = get_post_meta(get_the_ID(), 'seo_title', true);
	if(!empty($seo_title)) {
		$title = $seo_title;
	}
	// Lionmedia98:v002:END seo fix
	return $title;
}
...
function insert_seo_fields() {
	...
	$title = get_post_meta($post->ID,'title',true);
		...
		update_post_meta($post->ID, 'title', $seo_title);
		delete_post_meta($post->ID, 'seo_title');
		$title = $seo_title;
	}
	// Lionmedia98:v002:END SEO Fix
	...
	if(empty($description)) {
		...
		if(empty($description)) {
			$description = ganohrs_make_description(get_the_content(null, false));
		}
		if(!empty($description)) {
...
if (!function_exists('ganohrs_make_description')) {
	function ganohrs_make_description($description, $length = 158) {
		$description = preg_replace('/<!--more-->.+/is', '', $description); //moreタグ以降削除
		$description = preg_replace('#\[/?(red|blue|mark)\]#is', '', $description); // 装飾用タグは存在を消す
		$description = strip_shortcodes($description);//ショートコード削除
		$description = strip_tags($description);//タグの除去
		$description = preg_replace('/(https?|ftp)(:\\/\\/[-_.!~*\'()a-zA-Z0-9;\\/?:\\@&=+\\$,%#]+)/', '', $description); //URLを取り除く
		$description = preg_replace('/&nbsp;|[ \t]+|&#x(200[bcd]|feff);|\r\n|\r|\n/', '', $description);//特殊文字の削除（今回はスペースのみ）
		$description = mb_ereg_replace('。', '.', $description); //句読点処理
		$description = mb_ereg_replace('、', '', $description); //句読点処理
		$description = mb_ereg_replace('！', '!', $description); //句読点処理
		$description = mb_ereg_replace('‘|’|「|」|『|』【|】|（|）|\(|\)', '', $description); //カッコ処理
		$description = preg_replace('/\\[.+?\\]/i', '', $description); //ショートコードを取り除く
		$description = preg_replace('/pic\.twitter.+&mdash;/', '', $description); //URLを取り除く
...
function fit_ogp(){
	...
	if (is_singular()){
		...
		*/
		...
		// Lionmedia98:v002:START SEO Fix
		if(description_fit()){
			echo '<meta property="og:description" content="'.str_replace('"',"'",description_fit()).'" />'."\n";
		}elseif(have_posts()){
			while ( have_posts() ) {
				the_post();
				// $description = get_the_excerpt();
				$description = get_the_content(null, false);
				$description = ganohrs_make_description($description);
				echo '<meta property="og:description" content="'.str_replace('"',"'", $description) . '" />'."\n";
			}
		}
		// Lionmedia98:v002:END SEO Fix
	}elseif (is_home()){
		...
		*/
		...
		// Lionmedia98:v002:START SEO Fix
		if(get_option('fit_seo_descriptionTop')){
			echo '<meta property="og:description" content="'.str_replace('"',"'",get_option('fit_seo_descriptionTop')).'" />'."\n";
		}else{
			echo '<meta property="og:description" content="'.str_replace('"',"'",get_bloginfo('description')).'" />'."\n";
		}
		// Lionmedia98:v002:END SEO Fix
		...
		*/
		...
		// Lionmedia98:v002:START SEO Fix
		if (term_description()) {
			echo '<meta property="og:description" content="'.str_replace('"',"'",term_description()).'" />'."\n";
		}else{
			echo '<meta property="og:description" content="'.str_replace('"',"'",get_bloginfo('description')).'" />'."\n";
		}
		// Lionmedia98:v002:END SEO Fix
		if(is_year()){
			...
			*/
			// ORIGINAL:END
			// Lionmedia98:v002:START Bugfix
			$cat = get_the_category();
			if ($cat !== false && is_array($cat) && count($cat) > 0) {
				$cat_id = $cat[0]->cat_ID;
				echo '<meta property="og:url" content="'.get_category_link($cat_id).'" />'."\n";
			}
			// Lionmedia98:v002:END Bugfix
		}elseif(is_tag()){
			$tag = get_the_tags();
...
// ORIGINAL:END
// Lionmedia98:001:START Customize
add_action( 'widgets_init', function(){
	return register_widget( "AdWidgetItemClass" );
});
// Lionmedia98:001:END Customize
...
function fit_share_btn(){
	...
	if (true
		...
		&& (
			false
			|| (in_array('facebook' , $options) && $options['facebook'] )
			|| (in_array('twitter'  , $options) && $options['twitter']  )
			|| (in_array('hatebu'   , $options) && $options['hatebu']   )
			|| (in_array('pocket'   , $options) && $options['pocket']   )
			|| (in_array('line'     , $options) && $options['line']     )
		)
	) {
		// Lionmedia98:000:END Customize
		echo '<aside>'."\n";
		...
		// ORIGINAL:END
		// Lionmedia98:000:START Customize
		if ( in_array('facebook', $options) && $options['facebook'] ) {
			echo '<li class="socialList__item"><a class="socialList__link icon-facebook" href="http://www.facebook.com/sharer.php?u='. urlencode(get_permalink()) .'&amp;t='. urlencode(the_title("","",0)) .'" target="_blank" title="Facebookで共有"></a></li>';
		}
		if ( in_array('twitter', $options)  && $options['twitter']  ) {
			echo '<li class="socialList__item"><a class="socialList__link icon-twitter" href="http://twitter.com/intent/tweet?text='. urlencode(the_title("","",0)) .'&amp;'. urlencode(get_permalink()) .'&amp;url='. urlencode(get_permalink()) .'" target="_blank" title="Twitterで共有"></a></li>';
		}
		if ( in_array('hatebu', $options)   && $options['hatebu']   ) {
			echo '<li class="socialList__item"><a class="socialList__link icon-hatebu" href="http://b.hatena.ne.jp/add?mode=confirm&amp;url='. urlencode(get_permalink()) .'&amp;title='. urlencode(the_title("","",0)) .'" target="_blank" data-hatena-bookmark-title="'. urlencode(get_permalink()) .'" title="このエントリーをはてなブックマークに追加"></a></li>';
		}
		if ( in_array('pocket', $options)   && $options['pocket']   ) {
			echo '<li class="socialList__item"><a class="socialList__link icon-pocket" href="http://getpocket.com/edit?url='. urlencode(get_permalink()) .'" target="_blank" title="pocketで共有"></a></li>';
		}
		if ( in_array('line', $options)     && $options['line']     ) {
			echo '<li class="socialList__item"><a class="socialList__link icon-line" href="http://line.naver.jp/R/msg/text/?'. urlencode(the_title("","",0)) .'%0D%0A'. urlencode(get_permalink()) .'" target="_blank" title="LINEで送る"></a></li>';
		}
		// Lionmedia98:000:END Customize
		echo '</ul>'."\n";
...
// ORIGINAL:END
// Lionmedia98:000:START Customize
function gnr_putItemPosition($ipos) {
	return '<meta itemprop="position" content="' . $ipos . '" />';
}
function gnr_get_now_url() {
...
function fit_breadcrumb( $args = array() ){
	...
	if( !is_home() && !is_admin() ){
		...
		$str.= '<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a href="'. home_url() .'/" itemprop="item"><span class="icon-home" itemprop="name">'. $home .'</span></a>' . gnr_putItemPosition($ipos++) . '</li>';
		$my_taxonomy = get_query_var( 'taxonomy' );
		$cpt = get_query_var( 'post_type' );
		...
		if( $my_taxonomy && is_tax( $my_taxonomy ) ) {
			...
			$post_types = get_taxonomy( $my_taxonomy )->object_type;
			$cpt = $post_types[0];
			$str.='<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a href="' .get_post_type_archive_link( $cpt ).'" itemprop="item"><span itemprop="name">'. get_post_type_object( $cpt )->label.'</span></a>' . gnr_putItemPosition($ipos++) . '</li>';

			if( $my_tax -> parent != 0 ) {
				$ancestors = array_reverse( get_ancestors( $my_tax -> term_id, $my_tax->taxonomy ) );

				foreach( $ancestors as $ancestor ) {
					$str.='<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a href="'. get_term_link( $ancestor, $my_tax->taxonomy ) .'" itemprop="item"><span itemprop="name">'. get_term( $ancestor, $my_tax->taxonomy )->name .'</span></a>' . gnr_putItemPosition($ipos++) . '</li>';
				}
			}
			$str.='<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemprop="item" href="' . gnr_get_now_url() . '"><span itemprop="name">'. $my_tax -> name . '</span></a>' . gnr_putItemPosition($ipos++) . '</li>';
		} elseif( is_category() ) {
			$cat = get_queried_object();
			if( $cat -> parent != 0 ) {
				$ancestors = array_reverse( get_ancestors( $cat -> cat_ID, 'category' ));
				foreach( $ancestors as $ancestor ){
					$str.='<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a href="'. get_category_link( $ancestor ) .'" itemprop="item"><span itemprop="name">'. get_cat_name( $ancestor ) .'</span></a>' . gnr_putItemPosition($ipos++) . '</li>';
				}
			}
			$str.='<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemprop="item" href="' . gnr_get_now_url() . '"><span itemprop="name">'. $cat -> name . '</span></a>' . gnr_putItemPosition($ipos++) . '</li>';
		} elseif( is_post_type_archive() ) {
			$cpt = get_query_var( 'post_type' );
			$str.='<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemprop="item" href="' . gnr_get_now_url() . '"><span itemprop="name">'. get_post_type_object( $cpt )->label . '</span></a>' . gnr_putItemPosition($ipos++) . '</li>';
		} elseif( $cpt && is_singular( $cpt ) ) {
			$taxes = get_object_taxonomies( $cpt );
			$mytax = $taxes[0];
			$str.='<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a href="' .get_post_type_archive_link( $cpt ).'" itemprop="item"><span itemprop="name">'. get_post_type_object( $cpt )->label.'</span></a>' . gnr_putItemPosition($ipos++) . '</li>';
			...
			if (isset($taxes) && $taxes !== false) {
				...
				if( $tax -> parent != 0 ) {
					$ancestors = array_reverse( get_ancestors( $tax -> term_id, $mytax ) );
					foreach( $ancestors as $ancestor ) {
						$str.='<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a href="'. get_term_link( $ancestor, $mytax ).'" itemprop="item"><span itemprop="name">'. get_term( $ancestor, $mytax )->name . '</span></a>' . gnr_putItemPosition($ipos++) . '</li>';
					}
				}
				$str.='<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a href="'. get_term_link( $tax, $mytax ).'" itemprop="item"><span itemprop="name">'. $tax -> name . '</span></a>' . gnr_putItemPosition($ipos++) . '</li>';
			}
			$str.= '<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemprop="item" href="' . gnr_get_now_url() . '"><span itemprop="name">'. $post -> post_title .'</span></a>' . gnr_putItemPosition($ipos++) . '</li>';
		} elseif( is_single() ) {
			$categories = get_the_category( $post->ID );
			$cat = get_youngest_cat( $categories );
			if( $cat -> parent != 0 ) {
			...
			$str.='<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a href="'. get_category_link( $cat -> term_id ). '" itemprop="item"><span itemprop="name">'. $cat-> cat_name . '</span></a>' . gnr_putItemPosition($ipos++) . '</li>';
			$str.= '<li class="breadcrumb__item"><a itemprop="item" href="' . gnr_get_now_url() . '"><span itemprop="name">'. $post -> post_title .'</span></a>' . gnr_putItemPosition($ipos++) . '</li>';
		} elseif( is_page() ) {
			if( $post -> post_parent != 0 ) {
				$ancestors = array_reverse( get_post_ancestors( $post->ID ) );
				foreach( $ancestors as $ancestor ) {
					$str.='<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a href="'. get_permalink( $ancestor ).'" itemprop="item"><span itemprop="name">'. get_the_title( $ancestor ) .'</span></a>' . gnr_putItemPosition($ipos++) . '</li>';
				}
			}
			$str.= '<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemprop="item" href="' . gnr_get_now_url() . '"><span itemprop="name">'. $post -> post_title .'</span></a>' . gnr_putItemPosition($ipos++) . '</li>';
		} elseif( is_date() ) {
				if( get_query_var( 'day' ) != 0) {
					$str.='<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a href="'. get_year_link(get_query_var('year')). '" itemprop="item"><span itemprop="name">' . get_query_var( 'year' ). '年</span></a>' . gnr_putItemPosition($ipos++) . '</li>';
					$str.='<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a href="'. get_month_link(get_query_var( 'year' ), get_query_var( 'monthnum' ) ). '" itemprop="item"><span itemprop="name">'. get_query_var( 'monthnum' ) .'月</span></a>' . gnr_putItemPosition($ipos++) . '</li>';
					$str.='<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemprop="item" href="' . gnr_get_now_url() . '"><span itemprop="name">'. get_query_var('day'). '日</span></a>' . gnr_putItemPosition($ipos++) . '</li>';
			} elseif( get_query_var('monthnum' ) != 0) {
				$str.='<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a href="'. get_year_link( get_query_var('year') ) .'" itemprop="item"><span itemprop="name">'. get_query_var( 'year' ) .'年</span></a>' . gnr_putItemPosition($ipos++) . '</li>';
				$str.='<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemprop="item" href="' . gnr_get_now_url() . '"><span itemprop="name">'. get_query_var( 'monthnum' ). '月</span></a>' . gnr_putItemPosition($ipos++) . '</li>';
			} else {
				$str.='<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemprop="item" href="' . gnr_get_now_url() . '"><span itemprop="name">'. get_query_var( 'year' ) .'</span></a>年' . gnr_putItemPosition($ipos++) . '</li>';
			}
		} elseif( is_search() ) {
			$str.='<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemprop="item" href="' . gnr_get_now_url() . '"><span itemprop="name">「'. get_search_query() .'」'. $search .'</span></a>' . gnr_putItemPosition($ipos++) . '</li>';
		} elseif( is_author() ){
			$str .='<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemprop="item" href="' . gnr_get_now_url() . '"><span itemprop="name">'. $author . get_the_author_meta('display_name', get_query_var( 'author' )).'</span></a>' . gnr_putItemPosition($ipos++) . '</li>';
		} elseif( is_tag() ){
			$str.='<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemprop="item" href="' . gnr_get_now_url() . '"><span itemprop="name">'. $tag . single_tag_title( '' , false ). '</span></a>' . gnr_putItemPosition($ipos++) . '</li>';
		} elseif( is_attachment() ){
			$str.= '<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemprop="item" href="' . gnr_get_now_url() . '"><span itemprop="name">'. $post -> post_title .'</span></a>' . gnr_putItemPosition($ipos++) . '</li>';
		} elseif( is_404() ){
			$str.='<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemprop="item" href="' . gnr_get_now_url() . '"><span itemprop="name">'.$notfound.'</span></a>' . gnr_putItemPosition($ipos++) . '</li>';
		} else{
			$str.='<li class="breadcrumb__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemprop="item" href="' . gnr_get_now_url() . '"><span itemprop="name">'. wp_title( '', true ) .'</span></a>' . gnr_putItemPosition($ipos++) . '</li>';
		}
...
.gnr-tgl-details,.gnr-tgl-detailsnormal{border-radius:5px;clear:both;padding:5px 10px;background:#fbfbfb;color:#2299dd;border:1px solid #d0d0d0}.gnr-tgl-contents,.gnr-tgl-contentsnormal{margin-left:10px;padding:15px 20px}.gnr-tgl-summary,.gnr-tgl-summarynormal{border:none;list-style:none;cursor:pointer;font-size:120%;font-weight:bold;position:relative;display:block;line-height:40px}.gnr-tgl-summary::-webkit-details-marker,.gnr-tgl-summarynormal::-webkit-details-marker{display:none}.gnr-tgl-summary:before,.gnr-tgl-summarynormal:before{border-radius:3px;text-align:center;width:40px;height:40px;line-height:40px;display:inline-block;margin-right:0.5em;transition:.4s;border:solid 2px #2299dd;content:"＋"}details[open] .gnr-tgl-summary:before,details[open] .gnr-tgl-summarynormal:before{content:"－"}.gnr-tgl-summary:hover:before,.gnr-tgl-summarynormal:hover:before{background:#0279bf;color:white;transition:.4s}.gnr-tgl-detailsblack{border-radius:5px;clear:both;padding:5px 10px;background:black;color:white;border:1px solid white}.gnr-tgl-contentsblack{margin-left:10px;padding:15px 20px}.gnr-tgl-summaryblack{border:none;list-style:none;cursor:pointer;font-size:120%;font-weight:bold;position:relative;display:block;line-height:40px}.gnr-tgl-summaryblack::-webkit-details-marker{display:none}.gnr-tgl-summaryblack:before{border-radius:3px;text-align:center;width:40px;height:40px;line-height:40px;display:inline-block;margin-right:0.5em;transition:.4s;border:solid 2px white;content:"＋"}details[open] .gnr-tgl-summaryblack:before{content:"－"}.gnr-tgl-summaryblack:hover:before{background:white;color:black;transition:.4s}.gnr-tgl-detailsblue{border-radius:5px;clear:both;padding:5px 10px;background:blue;color:white;border:1px solid white}.gnr-tgl-contentsblue{margin-left:10px;padding:15px 20px}.gnr-tgl-summaryblue{border:none;list-style:none;cursor:pointer;font-size:120%;font-weight:bold;position:relative;display:block;line-height:40px}.gnr-tgl-summaryblue::-webkit-details-marker{display:none}.gnr-tgl-summaryblue:before{border-radius:3px;text-align:center;width:40px;height:40px;line-height:40px;display:inline-block;margin-right:0.5em;transition:.4s;border:solid 2px white;content:"＋"}details[open] .gnr-tgl-summaryblue:before{content:"－"}.gnr-tgl-summaryblue:hover:before{background:white;color:blue;transition:.4s}.gnr-tgl-detailsbrown{border-radius:5px;clear:both;padding:5px 10px;background:saddlebrown;color:white;border:1px solid brown}.gnr-tgl-contentsbrown{margin-left:10px;padding:15px 20px}.gnr-tgl-summarybrown{border:none;list-style:none;cursor:pointer;font-size:120%;font-weight:bold;position:relative;display:block;line-height:40px}.gnr-tgl-summarybrown::-webkit-details-marker{display:none}.gnr-tgl-summarybrown:before{border-radius:3px;text-align:center;width:40px;height:40px;line-height:40px;display:inline-block;margin-right:0.5em;transition:.4s;border:solid 2px white;content:"＋"}details[open] .gnr-tgl-summarybrown:before{content:"－"}.gnr-tgl-summarybrown:hover:before{background:white;color:brown;transition:.4s}.gnr-tgl-detailsfacebook{border-radius:5px;clear:both;padding:5px 10px;background:#3b5998;color:white;border:none}.gnr-tgl-contentsfacebook{margin-left:10px;padding:15px 20px}.gnr-tgl-summaryfacebook{border:none;list-style:none;cursor:pointer;font-size:120%;font-weight:bold;position:relative;display:block;line-height:40px}.gnr-tgl-summaryfacebook::-webkit-details-marker{display:none}.gnr-tgl-summaryfacebook:before{border-radius:3px;text-align:center;width:40px;height:40px;line-height:40px;display:inline-block;margin-right:0.5em;transition:.4s;border:solid 2px white;background-color:white;color:#3b5998;content:"＋"}details[open] .gnr-tgl-summaryfacebook:before{content:"－"}.gnr-tgl-summaryfacebook:hover:before{background:#3b5998;color:white;transition:.4s}.gnr-tgl-detailsgold{border-radius:5px;clear:both;padding:5px 10px;background:linear-gradient(to top left,#da8e00,#edac06,#f7de05);color:#7c6a03}.gnr-tgl-contentsgold{margin-top:10px;margin-left:10px;padding:15px 20px;border-radius:3px;background:linear-gradient(to top left,rgba(128,128,128,0.3),rgba(255,255,255,0.5))}.gnr-tgl-summarygold{border:none;list-style:none;cursor:pointer;font-size:120%;font-weight:bold;position:relative;display:block;line-height:40px}.gnr-tgl-summarygold::-webkit-details-marker{display:none}.gnr-tgl-summarygold:before{border-radius:3px;text-align:center;width:40px;height:40px;line-height:40px;display:inline-block;margin-right:0.5em;transition:.4s;background:linear-gradient(to top left,#da8e00,#edac06,#f7de05);color:#7c6a03;content:"＋"}details[open] .gnr-tgl-summarygold:before{content:"－"}.gnr-tgl-summarygold:hover:before{background:linear-gradient(to top left,#ea9e10,#fdbc16,#ffee15);color:white;transition:.4s}.gnr-tgl-detailsgray{border-radius:5px;clear:both;padding:5px 10px;background:#ccc;color:#555;border:1px solid #888}.gnr-tgl-contentsgray{margin-left:10px;padding:15px 20px}.gnr-tgl-summarygray{border:none;list-style:none;cursor:pointer;font-size:120%;font-weight:bold;position:relative;display:block;line-height:40px}.gnr-tgl-summarygray::-webkit-details-marker{display:none}.gnr-tgl-summarygray:before{border-radius:3px;text-align:center;width:40px;height:40px;line-height:40px;display:inline-block;margin-right:0.5em;transition:.4s;border:solid 2px white;background:white;color:#888;content:"＋"}details[open] .gnr-tgl-summarygray:before{content:"－"}.gnr-tgl-summarygray:hover:before{border:solid 2px white;background:#888;color:white;transition:.4s}.gnr-tgl-detailsgreen{border-radius:5px;clear:both;padding:5px 10px;background:lightgreen;color:#286c28;border:1px solid #286c28}.gnr-tgl-contentsgreen{margin-left:10px;padding:15px 20px}.gnr-tgl-summarygreen{border:none;list-style:none;cursor:pointer;font-size:120%;font-weight:bold;position:relative;display:block;line-height:40px}.gnr-tgl-summarygreen::-webkit-details-marker{display:none}.gnr-tgl-summarygreen:before{border-radius:3px;text-align:center;width:40px;height:40px;line-height:40px;display:inline-block;margin-right:0.5em;transition:.4s;border:solid 2px white;background-color:white;color:#286c28;content:"＋"}details[open] .gnr-tgl-summarygreen:before{content:"－"}.gnr-tgl-summarygreen:hover:before{background:#286c28;color:white;transition:.4s}.gnr-tgl-detailshatena{border-radius:5px;clear:both;padding:5px 10px;background:#2c6ebd;color:white;border:none}.gnr-tgl-contentshatena{margin-left:10px;padding:15px 20px}.gnr-tgl-summaryhatena{border:none;list-style:none;cursor:pointer;font-size:120%;font-weight:bold;position:relative;display:block;line-height:40px}.gnr-tgl-summaryhatena::-webkit-details-marker{display:none}.gnr-tgl-summaryhatena:before{border-radius:3px;text-align:center;width:40px;height:40px;line-height:40px;display:inline-block;margin-right:0.5em;transition:.4s;border:solid 2px white;background-color:white;color:#2c6ebd;content:"＋"}details[open] .gnr-tgl-summaryhatena:before{content:"－"}.gnr-tgl-summaryhatena:hover:before{background:#2c6ebd;color:white;transition:.4s}.gnr-tgl-detailsice{border-radius:5px;clear:both;padding:5px 10px;background:aliceblue;color:#618edf;border:1px solid cyan}.gnr-tgl-contentsice{margin-left:10px;padding:15px 20px}.gnr-tgl-summaryice{border:none;list-style:none;cursor:pointer;font-size:120%;font-weight:bold;position:relative;display:block;line-height:40px}.gnr-tgl-summaryice::-webkit-details-marker{display:none}.gnr-tgl-summaryice:before{border-radius:3px;text-align:center;width:40px;height:40px;line-height:40px;display:inline-block;margin-right:0.5em;transition:.4s;border:solid 2px aqua;color:aqua;content:"＋"}details[open] .gnr-tgl-summaryice:before{content:"－"}.gnr-tgl-summaryice:hover:before{background:aqua;color:white;transition:.4s}.gnr-tgl-detailsline{border-radius:5px;clear:both;padding:5px 10px;background:#00c300;color:white;border:none}.gnr-tgl-contentsline{margin-left:10px;padding:15px 20px}.gnr-tgl-summaryline{border:none;list-style:none;cursor:pointer;font-size:120%;font-weight:bold;position:relative;display:block;line-height:40px}.gnr-tgl-summaryline::-webkit-details-marker{display:none}.gnr-tgl-summaryline:before{border-radius:3px;text-align:center;width:40px;height:40px;line-height:40px;display:inline-block;margin-right:0.5em;transition:.4s;border:solid 2px white;background-color:white;color:#00c300;content:"＋"}details[open] .gnr-tgl-summaryline:before{content:"－"}.gnr-tgl-summaryline:hover:before{background:#00c300;color:white;transition:.4s}.gnr-tgl-detailsnavy{border-radius:5px;clear:both;padding:5px 10px;background:navy;color:white;border:1px solid cornflowerblue}.gnr-tgl-contentsnavy{margin-left:10px;padding:15px 20px}.gnr-tgl-summarynavy{border:none;list-style:none;cursor:pointer;font-size:120%;font-weight:bold;position:relative;display:block;line-height:40px}.gnr-tgl-summarynavy::-webkit-details-marker{display:none}.gnr-tgl-summarynavy:before{border-radius:3px;text-align:center;width:40px;height:40px;line-height:40px;display:inline-block;margin-right:0.5em;transition:.4s;border:solid 2px white;content:"＋"}details[open] .gnr-tgl-summarynavy:before{content:"－"}.gnr-tgl-summarynavy:hover:before{background:white;color:navy;transition:.4s}.gnr-tgl-detailspink{border-radius:5px;clear:both;padding:5px 10px;background:#fdf5f9;color:#d07;border:1px solid hotpink}.gnr-tgl-contentspink{margin-left:10px;padding:15px 20px}.gnr-tgl-summarypink{border:none;list-style:none;cursor:pointer;font-size:120%;font-weight:bold;position:relative;display:block;line-height:40px}.gnr-tgl-summarypink::-webkit-details-marker{display:none}.gnr-tgl-summarypink:before{border-radius:3px;text-align:center;width:40px;height:40px;line-height:40px;display:inline-block;margin-right:0.5em;transition:.4s;border:solid 2px hotpink;color:hotpink;content:"＋"}details[open] .gnr-tgl-summarypink:before{content:"－"}.gnr-tgl-summarypink:hover:before{background:hotpink;color:white;transition:.4s}.gnr-tgl-detailspocket{border-radius:5px;clear:both;padding:5px 10px;background:#ef4056;color:white;border:none}.gnr-tgl-contentspocket{margin-left:10px;padding:15px 20px}.gnr-tgl-summarypocket{border:none;list-style:none;cursor:pointer;font-size:120%;font-weight:bold;position:relative;display:block;line-height:40px}.gnr-tgl-summarypocket::-webkit-details-marker{display:none}.gnr-tgl-summarypocket:before{border-radius:3px;text-align:center;width:40px;height:40px;line-height:40px;display:inline-block;margin-right:0.5em;transition:.4s;border:solid 2px white;background-color:white;color:#ef4056;content:"＋"}details[open] .gnr-tgl-summarypocket:before{content:"－"}.gnr-tgl-summarypocket:hover:before{background:#ef4056;color:white;transition:.4s}.gnr-tgl-detailspurple{border-radius:5px;clear:both;padding:5px 10px;background:#94f;color:white}.gnr-tgl-contentspurple{margin-left:10px;padding:15px 20px}.gnr-tgl-summarypurple{border:none;list-style:none;cursor:pointer;font-size:120%;font-weight:bold;position:relative;display:block;line-height:40px}.gnr-tgl-summarypurple::-webkit-details-marker{display:none}.gnr-tgl-summarypurple:before{border-radius:3px;text-align:center;width:40px;height:40px;line-height:40px;display:inline-block;margin-right:0.5em;transition:.4s;border:solid 2px white;color:white;content:"＋"}details[open] .gnr-tgl-summarypurple:before{content:"－"}.gnr-tgl-summarypurple:hover:before{background:honeydew;color:mediumpurple;transition:.4s}.gnr-tgl-detailsred{border-radius:5px;clear:both;padding:5px 10px;background:#e00;color:white;border:1px solid white}.gnr-tgl-contentsred{margin-left:10px;padding:15px 20px}.gnr-tgl-summaryred{border:none;list-style:none;cursor:pointer;font-size:120%;font-weight:bold;position:relative;display:block;line-height:40px}.gnr-tgl-summaryred::-webkit-details-marker{display:none}.gnr-tgl-summaryred:before{border-radius:3px;text-align:center;width:40px;height:40px;line-height:40px;display:inline-block;margin-right:0.5em;transition:.4s;border:solid 2px white;content:"＋"}details[open] .gnr-tgl-summaryred:before{content:"－"}.gnr-tgl-summaryred:hover:before{background:white;color:#e00;transition:.4s}.gnr-tgl-detailsshock{border-radius:5px;clear:both;padding:5px 10px;background:linear-gradient(45deg,red,green,blue,orange,blue,green,red);color:white;border:4px solid transparent}.gnr-tgl-contentsshock{margin-top:10px;margin-left:10px;padding:15px 20px;border-radius:5px;background:linear-gradient(135deg,#fff4,#fff8,#fff4)}.gnr-tgl-summaryshock{border:none;list-style:none;cursor:pointer;font-size:120%;font-weight:bold;position:relative;display:block;line-height:40px}.gnr-tgl-summaryshock::-webkit-details-marker{display:none}.gnr-tgl-summaryshock:before{border-radius:3px;text-align:center;width:40px;height:40px;line-height:40px;display:inline-block;margin-right:0.5em;transition:.4s;border:solid 2px transparent;background:linear-gradient(135deg,#fff4,#fff8,#fff4);content:"＋"}details[open] .gnr-tgl-summaryshock:before{content:"－"}.gnr-tgl-summaryshock:hover:before{border:solid 2px white;background:red;transition:.4s}.gnr-tgl-detailstwitter{border-radius:5px;clear:both;padding:5px 10px;background:#1da1f2;color:white;border:none}.gnr-tgl-contentstwitter{margin-left:10px;padding:15px 20px}.gnr-tgl-summarytwitter{border:none;list-style:none;cursor:pointer;font-size:120%;font-weight:bold;position:relative;display:block;line-height:40px}.gnr-tgl-summarytwitter::-webkit-details-marker{display:none}.gnr-tgl-summarytwitter:before{border-radius:3px;text-align:center;width:40px;height:40px;line-height:40px;display:inline-block;margin-right:0.5em;transition:.4s;border:solid 2px white;background-color:white;color:#1da1f2;content:"＋"}details[open] .gnr-tgl-summarytwitter:before{content:"－"}.gnr-tgl-summarytwitter:hover:before{background:#1da1f2;color:white;transition:.4s}.gnr-tgl-detailsvivid{border-radius:5px;clear:both;padding:5px 10px;background:linear-gradient(45deg,red 33%,yellow 33%,yellow 67%,orange 67%,orange 100%);color:black;border:1px solid yellow}.gnr-tgl-contentsvivid{margin-top:10px;margin-left:10px;padding:15px 20px;border-radius:3px;background:rgba(255,128,128,0.5);border:1px solid yellow}.gnr-tgl-summaryvivid{border:none;list-style:none;cursor:pointer;font-size:120%;font-weight:bold;position:relative;display:block;line-height:40px}.gnr-tgl-summaryvivid:before{border-radius:3px;text-align:center;width:40px;height:40px;line-height:40px;display:inline-block;margin-right:0.5em;transition:.4s;border:solid 2px white;background:red;color:white;content:"＋"}details[open] .gnr-tgl-summaryvivid:before{content:"－"}.gnr-tgl-summaryvivid:hover:before{border:solid 2px orange;background:white;color:red;transition:.4s}.gnr-tgl-detailswhite{border-radius:5px;clear:both;padding:5px 10px;background:#fcfcfc;color:#666;border:1px solid #aaa}.gnr-tgl-contentswhite{margin-left:10px;padding:15px 20px}.gnr-tgl-summarywhite{border:none;list-style:none;cursor:pointer;font-size:120%;font-weight:bold;position:relative;display:block;line-height:40px}.gnr-tgl-summarywhite::-webkit-details-marker{display:none}.gnr-tgl-summarywhite:before{border-radius:3px;text-align:center;width:40px;height:40px;line-height:40px;display:inline-block;margin-right:0.5em;transition:.4s;border:solid 2px #ccc;color:#666;content:"＋"}details[open] .gnr-tgl-summarywhite:before{content:"－"}.gnr-tgl-summarywhite:hover:before{background:#666;color:white;transition:.4s}.gnr-tgl-detailsyellow{border-radius:5px;clear:both;padding:5px 10px;background:#ffe;color:#807811;border:1px solid gold}.gnr-tgl-contentsyellow{margin-left:10px;padding:15px 20px}.gnr-tgl-summaryyellow{border:none;list-style:none;cursor:pointer;font-size:120%;font-weight:bold;position:relative;display:block;line-height:40px}.gnr-tgl-summaryyellow::-webkit-details-marker{display:none}.gnr-tgl-summaryyellow:before{border-radius:3px;text-align:center;width:40px;height:40px;line-height:40px;display:inline-block;margin-right:0.5em;transition:.4s;border:solid 2px goldenrod;color:goldenrod;content:"＋"}details[open] .gnr-tgl-summaryyellow:before{content:"－"}.gnr-tgl-summaryyellow:hover:before{background:goldenrod;color:lemonchiffon;transition:.4s}
<?php
if ( get_theme_mod('fit_skin_theme')) {
...
function my_amp(){
	//AMPチェック
	$anp = get_option('fit_anp_check');
	if ($anp === false || $anp !== 'value2') {
		return false;
	}
	if ( empty($_GET['amp']) ) {
		return false;
	}
	if(is_singular() && $_GET['amp'] === '1'){
		return true;
	}
	return false;
}
...
function ganohrs_create_pager_links($need_next_title, $is_bottom = true) {
	// URLパターン
	if (!defined('GNR_URL_PATTERN')) {
		define('GNR_URL_PATTERN', '/blog\/([a-z0-9\-]+\/)([1-9][0-9]*\/)?/');
	}

	// ページネーションコード記憶用
	$code = "";

	// need_next_titleがtrueなら、「次は」ページネーションを表示
	$next_title_code = "";
	$title = "";
	$hash = "";
	if($need_next_title) {
		...
		if($page) {
			...
			$ptitle = ganohrs_get_paged_title($content, $page + 1);
			if(empty($ptitle)) {
				$page = false;
			} else {
				$title = "次は" . ($page + 1) . "頁目：<strong>$ptitle</strong>";
			}
		}
		...
		$args = array(
			...
			'separator' 	   => ' ',
			'previouspagelink' => '',
			'nextpagelink'	   => '<span>&raquo; ' . $title . '</span>',
			'echo'			   => 0
		);
		$next_title_code = wp_link_pages( $args );
		if ($page) {
			$hash = ganohrs_url_encode_anchor($ptitle);
			$next_title_code = preg_replace('/href="([^"]+)"/', "href=\"\\1#$hash\"", $next_title_code);
		}
	}
	...
	$args = array(
		...
		'next_or_number'   => 'next',
		'separator' 	   => ' ',
		'previouspagelink' => '<span>&laquo; 戻る</span>',
		'nextpagelink'	   => '',
		'echo'			   => 0
	);
	$code = $code . wp_link_pages( $args );
	...
	$args = array(
		...
		'separator' 	   => ' ',
		'previouspagelink' => '',
		'nextpagelink'	   => '<span>次へ &raquo;</span>',
		'echo'			   => 0
	);
	$code = $code . wp_link_pages( $args );

	if(strlen($code) > 0) {
		echo '<div class="page-link-label">↓ 続きや前の内容はこちらから ↓</div><div class="page-link">';
		if(strlen($next_title_code) > 0) {
			 echo '<div>'. $next_title_code . '</div>';
		}
		echo $code;
		echo '</div>';
		if ($is_bottom) {
			echo '<div class="page-link-after">※ この記事は無用な通信を抑えるため分割されています</div>';
		}
		echo '<div>&nbsp;</div>';
	}
}
...
//アクセス解析設定画面
//////////////////////////////////////////////////
function lionmedia98_cutomizer( $wp_customize ) {

	// セクション
	$wp_customize->add_section( 'lionmedia98_section', array(
		'title'     => 'Lionmedia98用設定',
		'priority'  => 1,
		'description' => '
		<style type="text/css">
		.customize-control-title{color:#0073AA;border-top: #BFBFBF 1px dotted;padding-top: 10px;margin-top: 10px;}
	...
	// SEO ---------------------------------------------------------------------
	// Google Analytics4を用いてアクセス計測
	$wp_customize->add_setting( 'lionmedia98_seo_ga4', array(
		'type'              => 'option',
		'default'           => 'ga4',
		'transport'         => 'postMessage',
		'sanitize_callback' => 'fit_sanitize_select',
	));
	$wp_customize->add_control( 'lionmedia98_seo_ga4', array(
		'section'           => 'lionmedia98_section',
		'settings'          => 'lionmedia98_seo_ga4',
		'label'             => '■SEO設定',
		'description'       => 'GoogleアナリティクスでGA4を用いるかUAを用いるか指定',
		'type'              => 'select',
		'choices'           => array(
			'ga4'       => 'GA4のみで計測(推奨)',
			'ga4_ua'    => 'GA4とユニバーサル・アナリティクスを併用',
			'ua'        => 'ユニバーサル・アナリティクスのみを利用(非推奨)',
		),
	));

	// GA4の測定IDを指定する
	$wp_customize->add_setting( 'lionmedia98_seo_ga4_id', array(
		'default'           => '',
		'type'              => 'option',
		'transport'         => 'postMessage',
		'sanitize_callback' => 'sanitize_text_field',
	));
	$wp_customize->add_control( 'lionmedia98_seo_ga4_id', array(
		'section'           => 'lionmedia98_section',
		'settings'          => 'lionmedia98_seo_ga4_id',
		'description'       => '<a href="https://support.google.com/analytics/answer/10089681?hl=ja" target="_blank" rel="noopener">GA4</a>の測定IDを指定',
		'type'              => 'text',
	));

	// Microsoft Clarityを利用可能にする
	$wp_customize->add_setting( 'lionmedia98_seo_msclarity_id', array(
		'default'           => '',
		'type'              => 'option',
		'transport'         => 'postMessage',
		'sanitize_callback' => 'sanitize_text_field',
	));
	$wp_customize->add_control( 'lionmedia98_seo_msclarity_id', array(
		'section'           => 'lionmedia98_section',
		'settings'          => 'lionmedia98_seo_msclarity_id',
		'description'       => '<a href="https://clarity.microsoft.com/" target="_blank" rel="noopener">Microsoft Clarity</a>のプロジェクトIDを指定(空欄なら未使用)',
		'type'              => 'text',
	));

	// タイトルカウンタを利用可能にする
	$wp_customize->add_setting( 'lionmedia98_seo_title_counter', array(
		'type'              => 'option',
		'default'           => '28',
		'transport'         => 'postMessage',
		'sanitize_callback' => 'sanitize_text_field',
	));
	$wp_customize->add_control( 'lionmedia98_seo_title_counter', array(
		'section'           => 'lionmedia98_section',
		'settings'          => 'lionmedia98_seo_title_counter',
		'description'       => 'タイトルに文字数カウンタを追加し、指定した文字数を超えた場合に赤文字に変更します<br>※ 0は無効',
		'type'              => 'number',
		'input_attrs'   => array(
			'step'      => '1',
			'min'       => '0',
	...
	// 概要文カウンタを利用可能にする
	$wp_customize->add_setting( 'lionmedia98_seo_description_counter', array(
		'type'              => 'option',
		'default'           => '80',
		'transport'         => 'postMessage',
		'sanitize_callback' => 'sanitize_text_field',
	));
	$wp_customize->add_control( 'lionmedia98_seo_description_counter', array(
		'section'           => 'lionmedia98_section',
		'settings'          => 'lionmedia98_seo_description_counter',
		'description'       => '概要文（description）に文字数カウンタを追加し、指定した文字数を超えた場合に赤文字に変更します<br>※ 0は無効',
		'type'              => 'number',
		'input_attrs'   => array(
			'step'      => '1',
			'min'       => '0',
...
/*
	// WordPressが自動的に生成する添付ファイルの概要ページを無効化する
	$wp_customize->add_setting( 'lionmedia98_disable_auto_attachment_page', array(
		'type'              => 'option',
		'default'           => true,
		'transport'         => 'postMessage',
		'sanitize_callback' => 'fit_sanitize_checkbox',
	));
	$wp_customize->add_control( 'lionmedia98_disable_auto_attachment_page', array(
		'section'           => 'lionmedia98_section',
		'settings'          => 'lionmedia98_disable_auto_attachment_page',
		'label'             => 'WordPressが自動的に生成する添付ファイルの概要ページを無効化する',
		'description'       => '※ 無用なページをインデックスさせないことで、各ページの相対的な重要度の低下を減らせます',
		'type'              => 'checkbox',
	));
*/
	// AMP用 -------------------------------------------------------------------
/*
	// AMP用追加CSS
	$wp_customize->add_setting( 'lionmedia98_amp_css', array(
		'type' => 'option',
		'sanitize_callback' => '',
	));
	// 記事内広告 コントロール
	$wp_customize->add_control( 'lionmedia98_amp_css', array(
		'section'   => 'lionmedia98_section',
		'settings'  => 'lionmedia98_amp_css',
		'label'     => '■AMP用設定',
		'description' => 'AMP用に追加のCSSを指定する<br>※ 上級者向け',
		'type'      => 'textarea',
	));

	// AMPページに通常記事用のリンクを設置する
	$wp_customize->add_setting( 'lionmedia98_amp_linktext', array(
		'type'              => 'option',
		'transport'         => 'postMessage',
		'sanitize_callback' => '',
	));
	$wp_customize->add_control( 'lionmedia98_amp_linktext', array(
		'section'           => 'lionmedia98_section',
		'settings'          => 'lionmedia98_amp_linktext',
		'description'       => 'AMPから通常記事へ誘導するためのリンク文言を設定する<br>※ 文言を設定した場合、ページの最上部へリンクを出力し、空欄ならリンクを出力しません',
		'type'              => 'text',
	));

	// 固定記事もAMP化する
	$wp_customize->add_setting( 'lionmedia98_alow_amp_singlepage', array(
		'type'              => 'option',
		'transport'         => 'postMessage',
		'sanitize_callback' => 'fit_sanitize_checkbox',
	));
	$wp_customize->add_control( 'lionmedia98_alow_amp_singlepage', array(
		'section'           => 'lionmedia98_section',
		'settings'          => 'lionmedia98_alow_amp_singlepage',
		'label'             => '固定記事もAMP化する',
		'type'              => 'checkbox',
	));
*/
/*
	...
	// 管理者には閲覧数を表示する
	$wp_customize->add_setting( 'lionmedia98_admin_views_access', array(
		'type'              => 'option',
		'default'           => 'show_views',
		'transport'         => 'postMessage',
		'sanitize_callback' => 'fit_sanitize_select',
	));
	$wp_customize->add_control( 'lionmedia98_admin_views_access', array(
		'section'           => 'lionmedia98_section',
		'settings'          => 'lionmedia98_admin_views_access',
		'label'             => '■管理者ツールバー設定',
		'description'       => '管理者には表示している記事の閲覧数を表示する',
		'type'              => 'select',
		'choices'           => array(
			'hide_views'    => '非表示',
			'show_views'    => '表示',
	...
	// 管理者にはIDを表示する
	$wp_customize->add_setting( 'lionmedia98_admin_views_id', array(
		'type'              => 'option',
		'transport'         => 'postMessage',
		'sanitize_callback' => 'fit_sanitize_checkbox',
	));
	$wp_customize->add_control( 'lionmedia98_admin_views_id', array(
		'section'           => 'lionmedia98_section',
		'settings'          => 'lionmedia98_admin_views_id',
		'label'             => '管理者には表示している投稿や固定記事のIDを表示する',
		'type'              => 'checkbox',
	));
*/
	...
	// カテゴリを複数件選択できない仕様を解消する
	$wp_customize->add_setting( 'lionmedia98_customize_multi_category', array(
		'type'              => 'option',
		'default'           => 'multi_cat',
		'transport'         => 'postMessage',
		'sanitize_callback' => 'fit_sanitize_select',
	));
	$wp_customize->add_control( 'lionmedia98_customize_multi_category', array(
		'section'           => 'lionmedia98_section',
		'settings'          => 'lionmedia98_customize_multi_category',
		'label'             => '■動作のカスタマイズ',
		'description'       => '複数のカテゴリを選択可能にする<br>※ マルチカテゴリを使用したい場合に選択',
		'type'              => 'select',
		'choices'           => array(
			'one_cat'   => 'シングルカテゴリ',
			'multi_cat' => 'マルチカテゴリ(推奨)',
		),
	));

	// jQueryのCDNを指定可能にする
	$wp_customize->add_setting( 'lionmedia98_jquery_cdn_url', array(
		'default'           => 'https://ajax.aspnetcdn.com/ajax/jQuery/jquery-3.6.4.min.js',
		'type'              => 'option',
		'transport'         => 'postMessage',
		'sanitize_callback' => 'sanitize_text_field',
	));
	$wp_customize->add_control( 'lionmedia98_jquery_cdn_url', array(
		'section'           => 'lionmedia98_section',
		'settings'          => 'lionmedia98_jquery_cdn_url',
		'description'       => 'jQuery・CDNのURLを指定する<br>※ 例:https://ajax.aspnetcdn.com/ajax/jQuery/jquery-3.6.4.min.js',
		'type'              => 'text',
	));

	// 不具合の多いFIT製目次生成フィルタを無効化する
	$wp_customize->add_setting( 'lionmedia98_disable_fit_toc', array(
		'type'              => 'option',
		'transport'         => 'postMessage',
		'sanitize_callback' => 'fit_sanitize_checkbox',
	));
	$wp_customize->add_control( 'lionmedia98_disable_fit_toc', array(
		'section'           => 'lionmedia98_section',
		'settings'          => 'lionmedia98_disable_fit_toc',
		'label'             => 'FIT製の目次生成フィルタを無効化する',
		'description'       => '※ 他の目次生成プラグインやLionmedia98用の目次生成機能を使用したい場合にチェック(投稿設定から目次を無効にしても不具合が出るためチェック推奨)',
		'type'              => 'checkbox',
	));
/*
	// 独自のブログカードを利用する
	$wp_customize->add_setting( 'lionmedia98_allow_blogcard', array(
		'type'              => 'option',
		'transport'         => 'postMessage',
		'sanitize_callback' => 'fit_sanitize_checkbox',
	));
	$wp_customize->add_control( 'lionmedia98_allow_blogcard', array(
		'section'           => 'lionmedia98_section',
		'settings'          => 'lionmedia98_allow_blogcard',
		'label'             => 'Lionmedia98独自のブログカードを利用する',
		'description'       => '※ URLだけの行を、リッチなブログカード（ページネーション考慮済み）に変換します',
		'type'              => 'checkbox',
	));

	// ページネーションを利用可能にする
	$wp_customize->add_setting( 'lionmedia98_allow_pagenation', array(
		'type'              => 'option',
		'transport'         => 'postMessage',
		'sanitize_callback' => 'fit_sanitize_checkbox',
	));
	$wp_customize->add_control( 'lionmedia98_allow_pagenation', array(
		'section'           => 'lionmedia98_section',
		'settings'          => 'lionmedia98_allow_pagenation',
		'label'             => 'ページネーションを有効化する',
		'description'       => '※ 記事を分割（ページネーション）したい場合にチェック',
		'type'              => 'checkbox',
	));

	// ページネーションを考慮した独自の目次生成機能を有効化する
	$wp_customize->add_setting( 'lionmedia98_change_fit_toc', array(
		'type'              => 'option',
		'transport'         => 'postMessage',
		'sanitize_callback' => 'fit_sanitize_checkbox',
	));
	$wp_customize->add_control( 'lionmedia98_change_fit_toc', array(
		'section'           => 'lionmedia98_section',
		'settings'          => 'lionmedia98_change_fit_toc',
		'label'             => 'Lionmedia98独自の目次生成機能を用いる',
		'description'       => '※ ページネーション考慮済みの目次生成機能を利用できます(独自ブログカード採用時には必須)',
		'type'              => 'checkbox',
	));

	// 目次を開いた状態にするか否か
	$wp_customize->add_setting( 'lionmedia98_toc_auto_open', array(
		'type'              => 'option',
		'transport'         => 'postMessage',
		'sanitize_callback' => 'fit_sanitize_checkbox',
	));
	$wp_customize->add_control( 'lionmedia98_toc_auto_open', array(
		'section'           => 'lionmedia98_section',
		'settings'          => 'lionmedia98_toc_auto_open',
		'label'             => '目次を自動的に開いた状態にする',
		'type'              => 'checkbox',
	));

	// 目次の件数が多い場合に自動的に閉じる
	$wp_customize->add_setting( 'lionmedia98_toc_auto_open_count', array(
		'type'              => 'option',
		'default'           => '10',
		'transport'         => 'postMessage',
		'sanitize_callback' => 'sanitize_text_field',
	));
	$wp_customize->add_control( 'lionmedia98_toc_auto_open_count', array(
		'section'           => 'lionmedia98_section',
		'settings'          => 'lionmedia98_toc_auto_open_count',
		'description'       => '目次の件数が指定値以上の場合に自動的に目次を閉じる※ 0は無効',
		'type'              => 'number',
		'input_attrs'   => array(
			'step'      => '1',
			'min'       => '0',
	...
	// 2ページ目以降で目次を自動的に閉じる
	$wp_customize->add_setting( 'lionmedia98_toc_auto_close_page', array(
		'type'              => 'option',
		'default'           => '28',
		'transport'         => 'postMessage',
		'sanitize_callback' => 'sanitize_text_field',
	));
	$wp_customize->add_control( 'lionmedia98_toc_auto_close_page', array(
		'section'           => 'lionmedia98_section',
		'settings'          => 'lionmedia98_toc_auto_close_page',
		'label'             => 'ページネーションされた記事で2ページ目以降は目次を自動的に閉じる',
		'description'       => '※ ページネーションされた記事で、その1ページ目では目次を開いておきますが、2ページ目以降では自動的に目次を閉じます',
		'type'              => 'checkbox',
	));

	// ページネーションされた2ページ目以降に広告を表示しない
	$wp_customize->add_setting( 'lionmedia98_pagenate_reduce_ad', array(
		'type'              => 'option',
		'transport'         => 'postMessage',
		'sanitize_callback' => 'fit_sanitize_checkbox',
	));
	$wp_customize->add_control( 'lionmedia98_pagenate_reduce_ad', array(
		'section'           => 'lionmedia98_section',
		'settings'          => 'lionmedia98_pagenate_reduce_ad',
		'label'             => 'ページネーションされた記事の2ページ目以降に広告を表示しない',
		'description'       => '※ ページネーションされた記事で、その1ページ目には広告を表示しますが、2ページ目以降では広告を表示しません',
		'type'              => 'checkbox',
	));

	// ページネーションの1ページ目を表示しないで途中のページへアクセスされた際の処理
	$wp_customize->add_setting( 'lionmedia98_pagenate_automove', array(
		'type'              => 'option',
		'transport'         => 'postMessage',
		'sanitize_callback' => 'fit_sanitize_checkbox',
	));
	$wp_customize->add_control( 'lionmedia98_pagenate_automove', array(
		'section'           => 'lionmedia98_section',
		'settings'          => 'lionmedia98_pagenate_automove',
		'label'             => 'ページネーションされた記事の途中のページに直アクセスされたら対策する',
		'description'       => '※ ページネーションされた記事で、その1ページ目を表示せずに途中のページへ直接アクセスされた場合に、CookieとJavaScriptを用いて自動的に1ページ目へ移動させます',
		'type'              => 'checkbox',
	));
*/
	// ソースコードのハイライトを利用する
	$wp_customize->add_setting( 'lionmedia98_allow_highlighting', array(
		'type'              => 'option',
		'default'           => true,
		'transport'         => 'postMessage',
		'sanitize_callback' => 'fit_sanitize_checkbox',
	));
	$wp_customize->add_control( 'lionmedia98_allow_highlighting', array(
		'section'           => 'lionmedia98_section',
		'settings'          => 'lionmedia98_allow_highlighting',
		'label'             => '自動的にソースコードのハイライトを行う',
		'description'       => '※ &lt;pre&gt;&lt;code&gt;～&lt;/pre&gt;&lt;/code&gt;が含まれる投稿へ自動的にhighlight.jsを読み込みます',
		'type'              => 'checkbox',
	));
}
add_action( 'customize_register', 'lionmedia98_cutomizer' );



function ganohrs_seo_counter() {
	$title_max = get_option('lionmedia98_seo_title_counter');
	$description_max = get_option('lionmedia98_seo_description_counter');

	if ($title_max === false) {
		$title_max = 28;
	} elseif ($title_max !== false && is_numeric($title_max)) {
		$title_max = intval($title_max);
	}

	if ($description_max === false) {
		$description_max = 80;
	} if ($description_max !== false && is_numeric($description_max)) {
		$description_max = intval($description_max);
	}
	if (!$title_max && !$description_max) {
		return;
	}
?>
<script type="text/javascript">
  ...
  jQuery(
	  function($) {
		...
		function updateTitleCounter() {
		  ...
		  if (titleLength > <?php echo $title_max ?> ) {
		...
		function updateSeoTitleCounter() {
		  var titleLength = strLength($('.inside input#title').val()).toString() / 2;
		  var stCounter = $('#title-seo-counter').text(titleLength);
		  if (titleLength > <?php echo $title_max ?> ) {
			stCounter.addClass( 'title-counter-length-over' );
		  } else {
			stCounter.removeClass( 'title-counter-length-over' );
		...
		$('.inside input#title')
			.before('<div id="title-seo-counter"></div>')
			.bind('keyup', updateSeoTitleCounter);
		updateSeoTitleCounter();
		// カウンタ更新関数
		function updateSeoDescriptionCounter() {
		  var titleLength = strLength($('#description').val()).toString() / 2;
		  var stCounter = $('#description-counter').text(titleLength);
		  if (titleLength > <?php echo $description_max ?> ) {
			stCounter.addClass( 'title-counter-length-over' );
		  } else {
			stCounter.removeClass( 'title-counter-length-over' );
		  }
		...
		$('#description')
			.before('<div id="description-counter"></div>')
			.bind('keyup', updateSeoDescriptionCounter);
		updateSeoDescriptionCounter();
	  });
</script>
<style type='text/css'>
  div#submitdiv {
...
<?php }
add_action( 'admin_head-post.php', 'ganohrs_seo_counter' );
add_action( 'admin_head-post-new.php', 'ganohrs_seo_counter' );


function ganohrs_append_if_need_highlight_js($content) {
	if(ganohrs_is_amp()) {
		return $content;
	}
	...
	if(preg_match('#highlight-js-style#', $content)) {
		return $content;
	}
	$need_highlight = get_option('lionmedia98_allow_highlighting');
	if ($need_highlight !== false && $need_highlight != 1) {
		return $content;
	}
	$highlight_js = <<<EOF
<link id="highlight-js-style" rel="stylesheet" href="//cdnjs.cloudflare.com/ajax/libs/highlight.js/11.7.0/styles/atom-one-light.min.css">
<script id="highlight-js-script" defer src="//cdnjs.cloudflare.com/ajax/libs/highlight.js/11.7.0/highlight.min.js"></script>
<script id="highlight-js-line-number-script" defer src="https://cdnjs.cloudflare.com/ajax/libs/highlightjs-line-numbers.js/2.8.0/highlightjs-line-numbers.min.js" integrity="sha512-axd5V66bnXpNVQzm1c7u1M614TVRXXtouyWCE+eMYl8ALK8ePJEs96Xtx7VVrPBc0UraCn63U1+ARFI3ofW+aA==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>
<script id="highlight-js-start-script" defer>window.addEventListener("load", function() { hljs.highlightAll(); hljs.initLineNumbersOnLoad(); });</script>
EOF;
	$append_copy_code = <<<EOF
<script id="highlight-js-copyscript" defer>let copyToClipboard=(element)=>{let ranges=[];let selection=window.getSelection();let range=document.createRange();let result=false;for(let i=0;i<selection.rangeCount;i+=1){ranges[i]=selection.getRangeAt(i)}range.selectNodeContents(element);selection.removeAllRanges();selection.addRange(range);result=document.execCommand("copy");selection.removeAllRanges();for(let i=0;i<ranges.length;i+=1){selection.addRange(ranges[i])}return result};let body=document.querySelector("body");let active=[];body.addEventListener("click",(e)=>{try{const target=e.target;if(!target.classList.contains('copy-button'))return;const pre=target.closest("pre");let result=false;if(active.indexOf(target)!==-1)return;if(pre){result=copyToClipboard(pre);target.innerText=(result?"COPIED!":"FAILED!");target.classList.add((result?"success":"failed"));active.push(target);setTimeout(()=>{let index=active.indexOf(target);target.className="copy-button";target.innerText="COPY CODE";if(index !==-1)active.splice(index,1)},2000)}}catch(e){}});hljs.addPlugin({'after:highlightElement':({el,result})=>{el.innerHTML='<button class="copy-button">COPY CODE</button>'+result.value}});</script>
EOF;
	return $content . "\n\n" . $highlight_js . $append_copy_code;;
}
add_filter( 'the_content', 'ganohrs_append_if_need_highlight_js', 99999);

if ( ! function_exists('ganohrs_get_uri_full') ) :
...
if ( ! function_exists('ganohrs_tail_pattern_matched') ) :
...
add_filter( 'wp_enqueue_scripts', 'ganohrs_change_jquery', PHP_INT_MAX );
function ganohrs_change_jquery( ){
	if(ganohrs_is_amp()) {
		return;
	}
	$jquery_url = get_option('lionmedia98_jquery_cdn_url');
	if (!isset($jquery_url) || strlen($jquery_url) === 0) {
		return;
	}
	...
	wp_enqueue_script( 'jquery', $jquery_url, array(), NULL, true );
}

function ganohrs_remove_hooks(){
	$remove_fit_toc = get_option('lionmedia98_disable_fit_toc');
	if ($remove_fit_toc) {
		remove_filter('the_content', 'add_outline');
	}

	$multi_category = get_option('lionmedia98_customize_multi_category');
	if ($multi_category === false || $multi_category === '' || $multi_category === 'multi_cat') {
		remove_action( 'admin_print_footer_scripts', 'limit_category_select' );
	}
}
add_action('after_setup_theme','ganohrs_remove_hooks');

if (! function_exists('ganohrs_url_encode_anchor')) {
	function ganohrs_url_encode_anchor($anchor)
	{
		$return = $anchor;
		...
		return 'hashid' . bin2hex(hash('crc32b', $return));
	}
}

if (! function_exists('ganohrs_get_pagenum')) {
	function ganohrs_get_pagenum($uri = false) {
		// URIを取得
		if ($uri === false || $uri === null || empty($uri)) {
			$uri = $_SERVER['REQUEST_URI'];
		}
		// URIから「?」以降を削除
		$ret = strpos($uri, '?');
		if($ret !== false && $ret > 0) {
			$uri = substr($uri, 0, $ret);
		}
		// URIから「#」以降を削除
		$ret = strpos($uri, '#');
		if($ret !== false && $ret > 0) {
		...
		// URIの最後のパターンが「/」じゃないなら補充
		if(!ganohrs_tail_pattern_matched($uri, '/')) {
			$uri .= '/';
		}

		// URIからベースURIとページ番号を取得
		if(!preg_match('"(?P<base>^/?.+?/)(?P<page>[1-9][0-9]*)?/?$"', $uri, $matches)) {
			// この形式になってない場合処理終了
			return false;
		}
		...
		// ベースページの投稿がなければ処理不要
		if(!url_to_postid( $base )) {
			return false;
		}

		// ページ番号がないなら1ページ目とみなす
		if(empty($page) || $page === '' || !is_numeric($page)) {
			return 1;
		}
...
function ganohrs_get_paged_title($content, $page) {
	if (!is_numeric($page) || !is_string($content)) {
		return false;
	}
	$res = preg_match_all('/<!--nextpage-->/', $content, $nextpages_pos, PREG_OFFSET_CAPTURE | PREG_SET_ORDER);
	if ($res !== false && $res != 0) {
		$offset = $nextpages_pos[$page - 2][0][1];
		$res = preg_match('!<h(\d)[^>]*>(.*?)</h\\1>!ism', $content, $matched, 0, $offset);
		if ($res !== false) {
			return str_replace([' ', '-'], ['', '―'], wp_strip_all_tags($matched[2]));
		}
	}
...
function ganohrs_get_paged_excerpt($content, $page) {
	if (!is_numeric($page) || !is_string($content)) {
		return false;
	}
	$res = preg_match_all('/<!--nextpage-->/', $content, $nextpages_pos, PREG_OFFSET_CAPTURE | PREG_SET_ORDER);
	if ($res !== false && $res != 0) {
		$offset = $nextpages_pos[$page - 2][0][1];
		$res = substr($content, $offset);
		if ($res !== false) {
			return str_replace([' ', '-'], ['', '―'], wp_strip_all_tags($res));
		}
	}
...
function ganohrs_get_autoad_script() {
	$script = get_option('lionmedia98_adsense_autoad_script');
...
// Lionmedia98:000:START Gutenberg タイトルに特殊記号が自動付与される 不具合対処
// https://stackoverflow.com/questions/69629919/obj-symbol-in-wordpress-url
...
// Lionmedia98:000:END Gutenberg タイトルに特殊記号が自動付与される 不具合対処
