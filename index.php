<?php get_header(); ?>
  ...
  if (is_home() && !is_paged() && get_option('fit_function_pickup') == 'value2') :
	  ...
	  */
	  // ORIGINAL:END
	  // Lionmedia98:000:START Bugfix
 	  $cat_id01 = false;
	  $cat_id02 = false;
	  $cat_id03 = false;
	  if (isset($cat_01) && is_array($cat_01) && count($cat_01) > 0) {
	 	$cat_id01 = $cat_01[0]->term_id ;
	  }
	  if (isset($cat_02) && is_array($cat_02) && count($cat_02) > 0) {
		$cat_id02 = $cat_02[0]->term_id ;
	  }
	  if (isset($cat_03) && is_array($cat_03) && count($cat_03) > 0) {
		$cat_id03 = $cat_03[0]->term_id ;
	  }
	  // Lionmedia98:000:END Bugfix
 ?>
  <div class="key<?php if(get_option('fit_function_pickupSp')): ?> u-none-sp<?php endif; ?>">
	<ul class="key__list">
	  ...
	  */
	  ...
	  ?>
	  <li class="key__item key__item-first">
		<span class="key__cat bgc<?php if (isset($cat_id01) && in_array($cat_id01, $cat_meta) && isset($cat_meta[$cat_id01])) { echo esc_html($cat_meta[$cat_id01]);} ?> u-txtShdw"><?php the_category(' ' ,'' ,$post_id01); ?></span>
		<a class="key__link" href="<?php echo get_permalink($post_id01); ?>">
		<h2 class="key__title u-txtShdw"><?php
		$result_obj = get_post($post_id01);
		if (isset($result_obj)) {
			echo $result_obj->post_title;
		}
		?></h2>
		</a>
	  </li>
	  <li class="key__item key__item-second">
		<span class="key__cat bgc<?php if (isset($cat_id02) && in_array($cat_id02, $cat_meta) && isset($cat_meta[$cat_id02])) { echo esc_html($cat_meta[$cat_id02]);} ?> u-txtShdw"><?php the_category(' ' ,'' ,$post_id02); ?></span>
		<a class="key__link" href="<?php echo get_permalink($post_id02); ?>">
		<h2 class="key__title u-txtShdw"><?php
		$result_obj = get_post($post_id02);
		if (isset($result_obj)) {
			echo $result_obj->post_title;
		}
		?></h2>
	  ...
	  <li class="key__item key__item-third">
		<span class="key__cat bgc<?php if (isset($cat_id03) && in_array($cat_id03, $cat_meta) && isset($cat_meta[$cat_id03])) { echo esc_html($cat_meta[$cat_id03]);} ?> u-txtShdw"><?php the_category(' ' ,'' ,$post_id03); ?></span>
		<a class="key__link" href="<?php echo get_permalink($post_id03); ?>">
		<h2 class="key__title u-txtShdw"><?php
		$result_obj = get_post($post_id03);
		if (isset($result_obj)) {
			echo $result_obj->post_title;
		}
		?></h2>
